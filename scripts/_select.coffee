class SelectModule
  ...
  on_option_click: (event) =>
    ...
    target.find('h2, .image, .params').fadeOut 'fast', =>
      ...
      target.find('.min_height').html(element.data('min_height'))
      target.find('.opt_height').html(element.data('opt_height'))
      target.find('.min_window').html(element.data('min_window'))
